...
with open(sys.argv[1], "w") as txt_file:
    ...
    for i in range(0,4):
        for j in range(0, 50):
            if i == 3:
                if j in [3, 11, 24, 31, 37, 43, 49]:
                    print(f"(assert (>= X_{i}_{j} {-90 - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {-90 + perturbation_eps}))\n", file=txt_file)
                elif j in [4, 10, 25, 30, 38, 44, 50]:
                    print(f"(assert (>= X_{i}_{j} {96.7 - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {96.7 + perturbation_eps}))\n", file=txt_file)
                else:
