...
with open(sys.argv[1], "w") as txt_file:
    ...
    print(f"; bottom flask and middle mantle heating temperature space continuity constraint for unnormalized data:", file=txt_file)
    # Declare Variables
    for i in range(0,4):
        for j in range(0, 50):
        ...
        for j in range(0, 50):
            if i == 2:
                if j in [3,5,11,17,30,38,46,47,48]:
                    print(f"(assert (>= X_{i}_{j} {150 - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {150 + perturbation_eps}))\n", file=txt_file)
                else:
                ...
                if j in [3,5,11,17,30,38,46,47,48]:
                    print(f"(assert (>= X_{i}_{j} {60 - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {60 + perturbation_eps}))\n", file=txt_file)
                else:
                    print(f"(assert (>= X_{i}_{j} {sample_ts[i][j] - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {sample_ts[i][j] + perturbation_eps}))\n", file=txt_file)
            else:
    ...
    for j in range(0, 50):
        print(f"    (and (>= X_2_{j} (+ X_3_{j} {eps})))", file=txt_file)
        print(f"    (and (>= X_3_{j} (+ X_2_{j} {+ eps})))", file=txt_file)
