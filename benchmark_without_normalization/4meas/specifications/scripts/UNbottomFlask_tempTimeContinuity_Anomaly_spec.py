...
with open(sys.argv[1], "w") as txt_file:
    ...
    print(f"; bottom flask temperature time continuity constraint for unnormalized data:", file=txt_file)
    # Declare Variables
    for i in range(0,4):
        for j in range(0, 50):
        ...
        for j in range(0, 50):
            if i == 2:
                if j in [3, 5, 9, 11, 24, 26, 37, 39]:
                    print(f"(assert (>= X_{i}_{j} {-300 - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {-300 + perturbation_eps}))\n", file=txt_file)
                elif j in [4, 10, 25, 38]:
                    print(f"(assert (>= X_{i}_{j} {98.1 - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {98.1 + perturbation_eps}))\n", file=txt_file)
                else:
