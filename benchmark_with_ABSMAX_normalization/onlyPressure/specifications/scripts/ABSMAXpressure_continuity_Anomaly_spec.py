...
with open(sys.argv[1], "w") as txt_file:
    ...
    print(f"; Pressure Continuity constraint for ABSMAX normalization:", file=txt_file)
    # Declare Variables
    for i in range(0,1):
        for j in range(0, 50):
    ...
    for i in range(0,1):
        for j in range(0, 50):
            if i == 0:
                if j in [3, 8, 15]:
                    print(f"(assert (>= X_{i}_{j} {10.35 - perturbation_eps}))", file=txt_file)
                    print(f"(assert (<= X_{i}_{j} {10.35 + perturbation_eps}))\n", file=txt_file)
                else:
